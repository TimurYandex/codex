...
def compute_ball_accelerations(
    ...
    2. Угловое ускорение (момент силы):
       ...
       Знак: положительная F_t (вправо) создаёт положительный момент
       (вращение против часовой), НО при ударе снизу Ft направлена влево (отрицательная),
       что создаёт отрицательное omega_dot (вращение по часовой) — ПРАВИЛЬНО!

    Ожидаемые эффекты:
    - Положительная Fn ускоряет мяч вверх
    - Отрицательная Ft (при ударе) ускоряет мяч влево и создаёт вращение по часовой
    ...
    # Угловое ускорение
    # F_t < 0 (влево) создаёт вращение по часовой (omega < 0)
    # F_t > 0 (вправо) создаёт вращение против часовой (omega > 0)
