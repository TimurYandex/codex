...
class GraphPanel:
    ...
    def draw_grid(
        ...
        for i in range(n_x_lines + 1):
            ...
            arcade.draw_line(
                px, self.plot_y, px, self.plot_y + self.plot_height, COLOR_GRID, 1
            )
        ...
        for i in range(n_y_lines + 1):
            ...
            arcade.draw_line(
                self.plot_x, py, self.plot_x + self.plot_width, py, COLOR_GRID, 1
            )
...
def draw_graphs(
    ...
    graphs = [
        ...
        (
            "Deflection (m)",
            [(p.time, p.deflection) for p in history.points],
            COLOR_DEF,
            ranges["def"],
        ),
        (
            "Slip (m/s)",
            [(p.time, p.slip) for p in history.points],
            COLOR_SLIP,
            ranges["slip"],
        ),
        (
            "Omega (rad/s)",
            [(p.time, p.omega) for p in history.points],
            COLOR_OMEGA,
            ranges["omega"],
        ),
        ("Vx (m/s)", [(p.time, p.v_x) for p in history.points], COLOR_VX, ranges["vx"]),
        ("Vy (m/s)", [(p.time, p.v_y) for p in history.points], COLOR_VY, ranges["vy"]),
    ]
